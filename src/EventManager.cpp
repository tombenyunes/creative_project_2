...
void EventManager::update(Controller* _controller, vector<GameObject*>* _gameobjects)
{
	if (tutorialEnabled) {
		...
		if (currentIndex >= 8 && !i8) {
			i8 = true;
			GameObject* object = new Object(ofVec2f(340, 0), 1000, 220, _controller);
		...
		if (currentIndex >= 11 && !i11) {
			i11 = true;
			GameObject* object = new Object(ofVec2f(294, 174), 4000, 115, _controller);
			_gameobjects->push_back(object);
		}
		...
		if (currentIndex >= 19 && !i19) {
			i19 = true;
			for (int i = 0; i < 30; i++) {
				GameObject* spring = new Springs(ofVec2f(ofRandom(-ofGetWidth() / 2, ofGetWidth() / 2), ofRandom(-ofGetHeight() / 2, ofGetHeight() / 2)), ofRandom(25, 50), ofRandom(25, 75), ofRandom(25, 50), ofRandom(25, 75), 2, 2, 22, _controller);
